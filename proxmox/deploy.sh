...
# ─── Template selection (Debian 12 preferred, Ubuntu 24.04 fallback) ──────────
hdr "Preparing LXC template"
...
TEMPLATE=$(pveam available --section system 2>/dev/null \
  | awk '{print $2}' \
  | grep -E '^debian-12-standard' | sort -V | tail -1)

CT_OSTYPE="debian"
if [[ -z "$TEMPLATE" ]]; then
  warn "No debian-12-standard template found, trying ubuntu-24.04..."
  TEMPLATE=$(pveam available --section system 2>/dev/null \
    | awk '{print $2}' \
    | grep -E '^ubuntu-24.04-standard' | sort -V | tail -1)
  CT_OSTYPE="ubuntu"
fi

[[ -z "$TEMPLATE" ]] && die "No suitable template found (tried debian-12, ubuntu-24.04). Run: pveam available --section system"
log "Using template: $TEMPLATE (ostype: $CT_OSTYPE)"
...
# Privileged container for reliable Docker + overlay filesystem support.
...
# Pass ostype into the container so the heredoc can pick the right repo
pct exec "$CTID" -- bash -c "echo '$CT_OSTYPE' > /tmp/.ostype"

pct exec "$CTID" -- bash -s << 'DOCKER_INSTALL'
set -e
export DEBIAN_FRONTEND=noninteractive

# Detect distro and codename from the OS itself (most reliable)
. /etc/os-release
DISTRO="${ID}"           # debian | ubuntu
CODENAME="${VERSION_CODENAME}"  # bookworm | noble | etc.
