...
export function MinecraftServer() {
  ...
  const handleCopyLogs = () => {
    const serverLines = logs?.lines ?? [];
    const cmdLines = commandLog.flatMap((e) => [`> ${e.cmd}`, ...(e.out ? [e.out] : [])]);
    const text = [...serverLines, ...cmdLines].join('\n');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-gray-900 rounded-lg shadow flex flex-col" style={{ minHeight: '420px' }}>
          <div className="px-4 py-2 border-b border-gray-700 flex items-center justify-between">
            <span className="text-xs text-gray-400 font-semibold tracking-wide uppercase">Console</span>
            <button
